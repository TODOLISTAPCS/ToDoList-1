...
public class AppModelTest {
  ...
  public void setUp() {
    ...
    for (int i=0; i<initialData.length; i++) {
      testArrayList.add(new ToDoItem(-1, initialData[i], initialDates[i]));
    }
  }
  ...
  public void testPutItem1() {
    ToDoItem newItem = new ToDoItem(-1, "This is added item 1","Mon 01/01");
  ...
  public void testPutItem2() {
    ToDoItem item;
    ToDoItem newItem = model.putItem(new ToDoItem(-1, "This is item 1", "01/01"));
    newItem = model.putItem(new ToDoItem(-1, "This is item 2","01/02"));
  ...
  */
  public AppModel addSampleItems(AppModel model) {
    for(int i = 0;i<initialData.length;i++){
      model.putItem (new ToDoItem(-1, initialData[i],initialDates[i]));
    }    
    return model;
  }
